...
router.post("/harvestPlant", async (req, res) => {
  try {
    ...
    let isPlantBatchGrowing = await mysqlLogic.verifyPlantBatchIsGrowing(
      plantBatchId
    );
  ...
  try {
    try {
      ...
      if (
        ...
        isNaN(parseInt(daysToMature)) ||
        parseInt(daysToMature) <= 0
      ) {
      ...
      if (
        currentSeedInventory === undefined ||
        isNaN(parseInt(currentSeedInventory)) ||
        parseInt(currentSeedInventory) < 0
      ) {
        sendInternalServerError(
          res,
          "Invalid value for current seed inventory."
        );
        return;
      }
      ...
      success = await mysqlLogic.updatePlantInfo(
        plantId,
        plantName,
        plantPicture,
        daysToMature,
...
/**
 * @swagger
 * /plant/plantYieldByMonth:
 *   get:
 *     summary: Get plant yield rate data. (still under edit.)
 *     tags: [Plant]
 *     description: Retrieve plant yield rate data.
 *     responses:
 *       200:
